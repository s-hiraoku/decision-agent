...
class AgreementJudge(Protocol):
    """評価時のセマンティック一致判定(§7)。"""
    def judge_core_issues(
        self, expected: tuple[str, ...], review: ArtifactReview
    ) -> tuple[str, ...]:  # 戻り値 = missed issues
        ...
    def judge_revision_direction(
...
class PreferenceRule:
    ...
    id: str = ""                    # "rule-" + uuid4 hex 先頭 8 桁。空なら load 時に採番
...
### 3.4 ArtifactReview への追加フィールド

```python
engine: str = ""          # "heuristic" | "llm:claude-opus-4-8" — レビューの由来を記録に残す
```

DecisionRecord に保存されるため、後から「LLM レビューと heuristic レビューで
delta の傾向がどう違うか」を JSONL から分析できる。`from_dict` は欠損を空文字で許容
(既存レコードとの互換)。
...
`violated_rule_id` により、どのルールが判定に効いたかを機械可読に残し、
§3.1 の `hit_count` 更新と `learned_signals` の生成
(`"checked preference rule: <id>"`)に使う。
...
class AgreementOutput(BaseModel):
    core_issue_results: list[CoreIssueResult]   # issue ごとに noticed: bool + evidence: str
...
5. **rules CLI のテスト。** candidate → approve → active、reject → 削除、を
   一時ファイルで検証する。
6. **手動スモーク(CI 外)。** `ANTHROPIC_API_KEY` がある環境でのみ
